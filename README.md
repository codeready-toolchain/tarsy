...
- **Usage & Estimated Cost**: Soft Est. $ next to session/execution token usage (enabled by default); dedicated Usage page for date-window fleet dig-in. Provider prompt-cache read/create tokens are persisted, priced, and shown on Usage totals / by-model and on trace LLM interactions. See [Session Usage Cost Estimation](docs/session-usage-cost.md) and [ADR-0026: Prompt Caching](docs/adr/0026-prompt-caching.md)
...
- `GET /api/v1/sessions/:id/status` -- Lightweight poll (single session-row lookup): status, summaries, labels, `error_message`, and review fields (`review_status`, `assignee`, `quality_rating`, `action_taken`, `investigation_feedback`)
...
- **Agents**: Define entirely in YAML -- name, MCP servers, custom instructions, LLM backend, skills. No Go code needed; behavior comes from the shared controller (iterating/single-shot/scoring) selected by `type`
