...
- **Usage & Estimated Cost**: Soft Est. $ next to session/execution token usage (enabled by default); dedicated Usage page for date-window fleet dig-in and estimated-cost charts. Provider prompt-cache read/create tokens are persisted, priced, and shown on Usage totals / by-model and on trace LLM interactions. See [Session Usage Cost Estimation](docs/session-usage-cost.md), [ADR-0026: Prompt Caching](docs/adr/0026-prompt-caching.md), and [ADR-0033: Usage Charts](docs/adr/0033-usage-charts.md)
