...
- **Hybrid retrieval** — vector similarity (pgvector HNSW) + keyword matching (tsvector/GIN) fused via Reciprocal Rank Fusion (RRF). Similarity threshold (0.45) discards low-quality matches, confidence weighting promotes human-reviewed memories, and temporal decay (90-day half-life) fades unreinforced ones. Project is the hard security filter; `alert_type` and `chain_id` provide minor soft boosts
