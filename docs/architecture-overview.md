...
- **Session list** with filtering by status, alert type, chain, date range, session labels (`label=` JSON contains), and full-text search (searches session fields + timeline event content via PostgreSQL FTS with GIN index). Historical list and session detail show stored `labels` as chips. See [ADR-0031: Session Labels](adr/0031-session-labels.md).
...
- **Session label maps**: Optional `label_maps` catalog plus `defaults.label_map` / `chain.label_map` (empty ≡ injected `builtin` watch/action/noise). See [ADR-0031](adr/0031-session-labels.md)
