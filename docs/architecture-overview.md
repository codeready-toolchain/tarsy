...
The Python service has zero orchestration state and zero MCP knowledge. It receives messages + config via gRPC, calls the LLM provider API, and streams response chunks back. The same Generate uses up to 3 total attempts when no chunks have been yielded yet — timeout, empty response, connection/protocol errors, HTTP 408, throttle 429, 404, and 5xx. Waits honor Retry-After / RetryInfo (capped at 60s) or full jitter; spend-cap 429s and hints above 60s fail fast as `provider_error`. Cache `400` uses a dedicated degrade path. Retry policy is hardcoded (no YAML). Exhausted retries surface to Go as `max_retries`. On eligible looping investigation-style calls it attaches Claude `cache_control` / GPT-5.6+ OpenAI implicit cache options when `GenerateRequest.prompt_cache` is set; all backends extract cache usage into `UsageInfo`. See [ADR-0026: Prompt Caching](adr/0026-prompt-caching.md), [ADR-0027: Transient LLM Outage Handling](adr/0027-llm-transient-outage.md), and [ADR-0028: LLM Retry Resilience](adr/0028-llm-retry-resilience.md).
...
- **Tool result summarization** -- enabled by default for all MCP servers; large results (>5K tokens) are automatically summarized by an LLM call before being sent back to the investigating agent (can be disabled per-server). Operators can optionally set `defaults.summarization.llm_provider` and `defaults.summarization.llm_backend` to a cheaper named provider. A per-MCP-server overlay can set its own `llm_provider` and `llm_backend` for that server; unset keeps the investigating agent's model. On summarization LLM error, TARSy walks `defaults.summarization.fallback_list` when set, otherwise the calling agent's effective fallback list, locally (skipping names already attempted on that call, without switching the investigator or emitting `provider_fallback`) before MCP fail-open / `search_past_sessions` fail-closed. An empty fallback list keeps immediate fail-open / fail-closed. See [ADR-0030: Named Fallback Lists](adr/0030-named-fallback-lists.md).
...
|----------|--------------|----------------|
| Google (default) | gemini-3.8-flash | 1M tokens |
| OpenAI | gpt-6-sol | 1.05M tokens |
| Anthropic | claude-sonnet-5 | 1M tokens |
| xAI | grok-4.7 | 500K tokens |
...
**Automatic provider fallback**: When a primary provider fails (after Python-level retries are exhausted), the Go controller switches to the next provider whose name has not already been attempted this execution (the primary starts on that list — fallback does not walk back). Which list an execution walks is resolved from a named catalog (`fallback_lists` + `fallback_list`); deprecated inline `fallback_providers` still loads. Triggers are error-code-aware: immediate for `max_retries` or missing credentials; after two consecutive failures for other transient errors (including spend-cap 429s that Python did not retry), with consecutive counters reset after a successful iterating LLM call. No-partial provider errors are recorded as timeline `error` events and are not appended to the model conversation. Adaptive streaming timeouts (initial response, stall detection, max call) reduce time wasted on unresponsive providers. Fallback events are recorded in the timeline and surfaced in the dashboard. See [ADR-0003: LLM Provider Fallback](adr/0003-llm-provider-fallback.md), [ADR-0030: Named Fallback Lists](adr/0030-named-fallback-lists.md), [ADR-0027: Transient LLM Outage Handling](adr/0027-llm-transient-outage.md), and [ADR-0028: LLM Retry Resilience](adr/0028-llm-retry-resilience.md).
...
Provider prompt caching (Claude `cache_control`, GPT-5.6+ OpenAI implicit looping cache, Gemini implicit) is measured and priced: `input_tokens` is uncached input; cache read/create are stored and billed separately. Trace LLM list/detail show per-call cache counts; Usage totals and by-model SUMs them. Session list / header / `ExecutionOverview` do not. Cluster kill switch: `system.prompt_caching.enabled` (default on; does not disable Gemini implicit caching).
