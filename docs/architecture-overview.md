...
- **Tool result summarization** -- enabled by default for all MCP servers; large results (>5K tokens) are automatically summarized by an LLM call before being sent back to the investigating agent (can be disabled per-server). Operators can optionally set `defaults.summarization.llm_provider` (and an optional per-MCP-server overlay) to a cheaper named provider; unset keeps the investigating agent's model.
