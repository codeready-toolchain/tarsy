...
| Q9 | Snapshot / catalog | Snapshot holds durable rates; promotions hold time windows | Clear separation so airgap after promo end does not keep intro pricing by policy. Escape hatch: price missing models via a dated promotion or temporary override. Time-bounded snapshot entries (GPT-5.6 Sol, Gemini 3.7/3.8 Flash intro) set `tarsy_rates_valid_until` so they become unpriced after the exclusive UTC bound unless catalog/override/promotion still match. Gemini 3.6 Flash snapshot stays durable GA. Rejected: bake new promo windows into the snapshot as general policy. |
