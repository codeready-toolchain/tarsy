...
| Q4 | Claude breakpoints | Sticky last-tool schema + system + **first user** + last **tool result** (if any). Skip the tool-schema / tool-result markers when there are no tools. Do not mark last-message or assistant | Same four-slot layout as GPT-5.6 explicit (Q11). Anthropic/Vertex lookback only finds prior writes at blocks that still carry `cache_control`; moving the marker off the previous last message restyles that prefix and yields 2× writes with ~0 reads. Forced conclusion leaves the extra user unmarked so the looping prefix can be read. Rejected: last message (Vertex looping wrote the full prompt every turn); last tool only; tools + system only. |
...
| Q9 | System-prompt layout | Leave layout in v1. Intra-session looping is the win | Tier 0 wall-clock time sits first in the system prompt, so cross-session reuse of skills + tools will not hit. Reorder/split is golden-prompt churn for little token volume. Sticky first-user + last-tool-result breakpoints cache intra-session turns. |
...
flowchart TD
  ...
  py -->|LangChain Claude / Vertex Claude| claude[cache_control on last tool schema + system + first user + last tool result]
...
3. Put `cache_control` on the **first** `role=user` message (sticky original alert) and the last `role=tool` message, if any. Do not mark assistant / tool-call-only messages. Forced conclusion keeps the same markers and leaves the conclusion user unmarked so Claude can read the looping prefix without a 2× write of the conclusion prompt.
