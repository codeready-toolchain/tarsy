...
2. **Average estimated cost per session** — one column per calendar day, the stat-card ratio.
...
| Q3 | Average chart | Per-day columns | Column = that day's cost / that day's session count, including sessions with no LLM rows. The value axis starts at $0 so height is the cost. Days with no sessions have no column, so a quiet day does not read as a $0 session. A day with sessions and $0 spend keeps a short column on the baseline. A day with exactly one session uses an outlined column, and the tooltip says the figure is that session's cost. Every tooltip includes the session count. The chart is not split by model. Rejected: a line (these are separate daily ratios, and a line reads as a path between them), a running average (lands on the card and dilutes spikes), confidence bands, and a fade for small counts beyond the single-session outline. |
...
| Q8 | Chart library | `@mui/x-charts` v9 (community) | Same major as Material UI 9 and the date pickers, so shared MUI internals stay aligned. Stacked area and columns are in the community package. Rejected: Recharts (a second styling system) and hand-rolled SVG. v8 depends on older MUI utils and does not belong next to the v9 date pickers. |
...
**Average chart.** One column per day. Each column is that day's estimated cost divided by that day's session count. The value axis starts at $0. Days with zero sessions have no column. A day with sessions and $0 spend keeps a short column on the baseline. The tooltip always includes the session count. A day with exactly one session uses an outlined column. Days with two or more sessions use a filled column.
...
- Days inside the window with no sessions are still returned: `session_count: 0`, `estimated_cost_usd: 0`, `average_cost_usd` omitted, no model costs. The client draws no average column when `session_count == 0`.
...
- Per-bucket completeness markers, confidence intervals, and a fade for small session counts beyond the single-session outlined column.
