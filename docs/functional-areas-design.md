...
TARSy supports both built-in components and YAML-based configuration, enabling system extension without code changes. Configuration defines agents, chains, MCP servers, LLM providers, named fallback lists, named label maps, and their relationships.
...
- **Named label maps**: Top-level `label_maps` catalog; `defaults.label_map` / `chain.label_map` are last-non-empty selectors (empty inherits; all empty → `builtin`). This is a session-level selector next to `fallback_list`, not a field on the `executive_summary` job block. See [ADR-0031](adr/0031-session-labels.md).
...
Compose and executive summary use nested `compose` / `executive_summary` blocks (`llm_provider`, `llm_backend`, `fallback_list`) on defaults and chains. Last non-empty wins: `defaults.llm_provider` → `chain.llm_provider` → `defaults.compose` / `defaults.executive_summary` → `chain.compose` / `chain.executive_summary`. `defaults.compose` beats `chain.llm_provider` (same for exec summary) so a mid-tier job default is not overridden by the chain's investigation model. Follow-up chat uses `chain.chat` then chain then defaults. Deprecated flat `compose_*` / `executive_summary_*` keys still load into the nested blocks (startup warning); mixing nested + deprecated keys on the same node is a load-time error.

**Fallback lists** are resolved by expanding a named catalog entry (`fallback_list`) or a deprecated inline `fallback_providers` slice, then taking the last non-nil layer. Investigation / dispatched-agent order (lowest to highest): `defaults` → chain → `defaults.agents.<name>` → stage → stage-agent / sub-agent ref. Side paths (chat, scoring, compose, exec summary) inherit defaults → chain when unset; dedicated job knobs (`defaults.scoring.fallback_list`, `defaults.compose.fallback_list`, `defaults.executive_summary.fallback_list`, …) beat the investigation list. Stage lists do not leak into chat/scoring/exec summary/compose. See [ADR-0003](adr/0003-llm-provider-fallback.md) and [ADR-0030](adr/0030-named-fallback-lists.md).
...
- Model caching by `(provider, model, api_key_env)` tuple (cache policy is **not** on the shared instance; Claude `cache_control` / OpenAI `prompt_cache_options` are per-request when `prompt_cache` is true; GPT-5.6+ ineligible calls still bind `prompt_cache_options` with no breakpoints)
...
|--------|----------|---------|
| GET | `/api/v1/sessions` | Paginated session list with filtering (`label=` contains a stored canonical name) |
...
| GET | `/api/v1/usage/series` | Per-day estimated cost for that window (charts; same filters plus timezone) |
...
- `web/dashboard/src/components/common/SessionLabelChips.tsx` -- Display-only chips for stored session labels
