...
func TestUsageSeriesHandler_Validation(t *testing.T) {
	s := &Server{}
	validWindow := "start_date=2024-01-01T00:00:00Z&end_date=2024-02-01T00:00:00Z"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			e := echo.New()
			req := httptest.NewRequest(http.MethodGet, "/api/v1/usage/series?"+tt.query, nil)
			rec := httptest.NewRecorder()
			c := e.NewContext(req, rec)

			err := s.usageSeriesHandler(c)
			if assert.Error(t, err) {
				he, ok := err.(*echo.HTTPError)
				if assert.True(t, ok, "expected echo.HTTPError") {
					assert.Equal(t, tt.wantErr, he.Code)
	...
	t.Run("missing or unknown timezone falls back to UTC", func(t *testing.T) {
		client := testdb.NewTestClient(t)
		server := &Server{sessionService: newUsageTestSessionService(client.Client)}
		...
		for _, tz := range []string{"", "Not/AZone", "Local"} {
			t.Run(tz, func(t *testing.T) {
				...
				e := echo.New()
				req := httptest.NewRequest(http.MethodGet, "/api/v1/usage/series?"+query, nil)
				rec := httptest.NewRecorder()
				c := e.NewContext(req, rec)

				err := server.usageSeriesHandler(c)
				require.NoError(t, err)
				assert.Equal(t, http.StatusOK, rec.Code)

				var resp models.UsageSeriesResponse
				require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &resp))
				assert.True(t, resp.CostEstimationEnabled)
				assert.Equal(t, "UTC", resp.Timezone)
				assert.Equal(t, models.UsageBucketDay, resp.Bucket)
				assert.NotEmpty(t, resp.Points)
			})
		}
	})

	t.Run("estimation disabled omits the series payload", func(t *testing.T) {
		svc := newUsageTestSessionService(&ent.Client{})
		svc.SetCostEstimationEnabled(false)
		server := &Server{sessionService: svc}

		e := echo.New()
		req := httptest.NewRequest(http.MethodGet, "/api/v1/usage/series?"+validWindow+"&timezone=Not/AZone", nil)
		rec := httptest.NewRecorder()
		c := e.NewContext(req, rec)
		...
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.JSONEq(t, `{"cost_estimation_enabled":false}`, rec.Body.String())
	})
}
