...
func TestGetTriageGroupHandler_InvalidParams(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "page_size non-numeric", path: "/api/v1/sessions/triage/reviewed?page_size=xyz"},
		{name: "page_size zero", path: "/api/v1/sessions/triage/reviewed?page_size=0"},
		{name: "page_size exceeds max", path: "/api/v1/sessions/triage/reviewed?page_size=999"},
	}
